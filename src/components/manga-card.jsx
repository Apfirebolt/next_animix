...
const MangaCard = ({ manga }) => {
  ...
  const formattedRating = averageRating ? (parseFloat(averageRating) / 10).toFixed(1) : null;
      ...
      <div className="relative aspect-[3/4] w-full overflow-hidden bg-neutral-100 dark:bg-neutral-800">
        {imageUrl ? (
          ...
          <div className="flex h-full w-full items-center justify-center text-neutral-400">
            <HiOutlineBookOpen className="h-10 w-10 stroke-1" />
          </div>
        )}
        ...
        <div className="mt-4 flex items-center justify-between border-t border-neutral-100 pt-3 text-xs dark:border-neutral-800/80">
          <div className="flex items-center gap-1 text-neutral-600 dark:text-neutral-400">
            ...
            <span className="font-medium text-neutral-900 dark:text-neutral-200">
              {chapterCount ? `${chapterCount} chs` : "Ongoing"}
            </span>
          </div>

          <span
            className={`rounded-full border px-2 py-0.5 text-[10px] font-semibold uppercase tracking-wider capitalize ${getStatusColor(
              status
            )}`}
          >
            {status || "Unknown"}
